...
void parseLine(const std::string &line) {
    ...
    // Extract only the first word for proper formatting
    p.command = extractCommand(p.command);

    processes.push_back(p);

    // Store correct parent process to avoid "Unknown"
    if (parent_map.find(p.ppid) == parent_map.end()) {
        parent_map[p.ppid] = "Unknown";  
    }

    // Only store valid parents, avoiding "Unknown"
    if (!p.command.empty()) {
...
void filterProcesses(const std::string &query) {
    for (const auto &p : processes) {
        if (p.command.find(query) != std::string::npos) {
            ...
            // Correct unknown parent handling
            if (parentCommand == "Unknown") {
                continue; // Skip if parent is unknown
            }
